...
#define SLE_UART_SEND_DATA_FLOW_DELAY_MS       35
...
{
    sample_at_log_print("%s send data complete conn_id:%u, link_state:%u\r\n", SLE_UART_SERVER_LOG,
        conn_id, link_state);
...
static errcode_t sle_tm_register_cbks(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_tm_register_cbks, sle_transmission_register_callbacks fail :%x\r\n",
            SLE_UART_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
    ...
    ssaps_write_callback)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_uart_server_init,sle_tm_register_cbks fail :%x\r\n", SLE_UART_SERVER_LOG, ret);
        return ret;
    }
