...
#define ERROR_SECURITY_INVALID_PARAM               0x80001501  /* input param's value is not in valid range. */
#define ERROR_SECURITY_PARAM_IS_NULL               0x80001502  /* input param is NULL and required not NULL. */
#define ERROR_SECURITY_NOT_INIT                    0x80001503  /* call other functions before call init function. */
#define ERROR_SECURITY_UNSUPPORT                   0x80001504  /* some configuration is unsupport. */
#define ERROR_SECURITY_UNEXPECTED                  0x80001505  /* reture when unexpected error occurs. */
#define ERROR_SECURITY_TRY_TIMES                   0x80001506  /* return when the number of try attempts failed. */
#define ERROR_SECURITY_CHN_BUSY                    0x80001507  /* try to create channel but all channels are busy. */
#define ERROR_SECURITY_CTX_CLOSED                  0x80001508  /* using one ctx to do something but has been closed. */
#define ERROR_SECURITY_NOT_SET_CONFIG              0x80001509  /* not set_config but need for symc. */
#define ERROR_SECURITY_NOT_ATTACHED                0x8000150A  /* not attach but need for symc. */
#define ERROR_SECURITY_NOT_SET_SESSION_KEY         0x8000150B  /* return when not set_session_key but need for klad. */
#define ERROR_SECURITY_NOT_MAC_START               0x8000150C  /* not mac_start but need for symc. */
#define ERROR_SECURITY_INVALID_HANDLE              0x8000150D  /* pass one invalid handle. */
#define ERROR_SECURITY_GET_PHYS_ADDR               0x8000150E  /* transfer from virt_addr to phys_addr failed. */
#define ERROR_SECURITY_SYMC_LEN_NOT_ALIGNED        0x8000150F  /* length isn't aligned to 16-Byte except CTR/CCM/GCM. */
/* the phys_addr writing to register is not aligned to 4-Byte. */
#define ERROR_SECURITY_SYMC_ADDR_NOT_ALIGNED       0x80001510
#define ERROR_SECURITY_PKE_RSA_SAME_DATA           0x80001511  /* rsa exp_mod  the input is equal to output. */
#define ERROR_SECURITY_PKE_RSA_CRYPTO_V15_CHECK    0x80001512  /* rsa crypto v15 padding check failed. */
#define ERROR_SECURITY_PKE_RSA_CRYPTO_OAEP_CHECK   0x80001513  /* rsa crypto oaep padding check failed. */
#define ERROR_SECURITY_PKE_RSA_VERIFY_V15_CHECK    0x80001514  /* rsa verify v15 padding check failed. */
#define ERROR_SECURITY_PKE_RSA_VERIFY_PSS_CHECK    0x80001515  /* rsa verify pss padding check failed. */
#define ERROR_SECURITY_PKE_RSA_GEN_KEY             0x80001516  /* rsa generate key failed. */
#define ERROR_SECURITY_PKE_SM2_DECRYPT             0x80001517  /* return when sm2 decrypt failed. */
#define ERROR_SECURITY_ECDSA_SIGN_R_IS_ZERO        0x80001518  /* return when ecdsa sign, the sig_r is zero. */
#define ERROR_SECURITY_ECDSA_SIGN_S_IS_ZERO        0x80001519  /* return when ecdsa sign, the sig_s is zero. */
#define ERROR_SECURITY_PKE_ECDSA_VERIFY_CHECK      0x8000151A  /* ecdsa verify check failed. */
#define ERROR_SECURITY_OTP_ADDR_NOT_ALIGNED        0x8000151B  /* return when otp address is not aligned to 4. */
#define ERROR_SECURITY_HASH_ADDR_NOT_ALIGNED       0x8000151C  /* return when hash data's address is not aligned */
#define ERROR_SECURITY_HASH_LEN_NOT_ALIGNED        0x8000151D  /* return when hash length is not block size aligned. */
#define ERROR_SECURITY_PKE_SECURITY_STRENGTH_NOT_MATCH 0x8000151E /* the security strength is not match. */
...
#define ERROR_SECURITY_MEMSET_S                    0x80001544  /* call memset_s failed. */
...
#define ERROR_SECURITY_OTP_PERMISSION              0x800015A5  /* return when otp access is forbidden. */
#define ERROR_SECURITY_KM_LOGIC                    0x800015A6  /* return when km logic's error occurs. */
#define ERROR_SECURITY_KM_KL_LOGIC                 0x800015A7  /* return when km HKL logic's error occurs. */
#define ERROR_SECURITY_KM_KC_LOGIC                 0x800015A8  /* return when km KEY CTRL logic's error occurs. */
#define ERROR_SECURITY_KLAD_ROOTKEY_NOT_READY      0x800015A9  /* return when klad rootkey is not ready. */
...
#define ERROR_SECURITY_PKE_ROBUST_WARNING          0x800015B8  /* pke robust warning. */
#define ERROR_SECURITY_RKP_LOCK_TIMEOUT            0x800015B9  /* return when rkp lock timeout. */
#define ERROR_SECURITY_RKP_CALC_TIMEOUT            0x800015BA  /* return when rkp calculate timeout. */
#define ERROR_SECURITY_KLAD_COM_ROUTE_TIMEOUT      0x800015BB  /* return when klad com route timeout. */
#define ERROR_SECURITY_OTP_TIMEOUT                 0x800015BC  /* return when otp wait timeout. */
#define ERROR_SECURITY_KEYSLOT_TIMEOUT             0x800015BD  /* return when keyslot wait timeout. */
#define ERROR_SECURITY_KEYSLOT_LOCK                0x800015BE  /* return when keyslot lock or unlock fail. */
#define ERROR_SECURITY_KLAD_TIMEOUT                0x800015BF  /* return when klad wait timeout. */
#define ERROR_SECURITY_KLAD_LOCK                   0x800015C0  /* return when klad lock or unlock fail. */
#define ERROR_SECURITY_HASH_LOCK_TIMEOUT           0x800015C1  /* return when hash lock wait timeout. */
#define ERROR_SECURITY_PKE_ALG_TIMEOUT             0x800015C2  /* return when pke algorithm timeout. */
