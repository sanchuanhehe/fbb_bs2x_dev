...
/**
 * @if Eng
 * @brief Struct of read by type request information.
 * @else
 * @brief 基于类型读请求信息。
 * @endif
 */
typedef struct {
    uint16_t request_id;    /*!< @if Eng Request id.
                                 @else   请求id。 @endif */
    uint16_t start_handle;  /*!< @if Eng Attribute start handle of the read request.
                                 @else   请求读的属性起始句柄。 @endif */
    uint16_t end_handle;    /*!< @if Eng Attribute end handle of the read request.
                                 @else   请求读的属性结束句柄。 @endif */
    bool need_rsp;          /*!< @if Eng Whether response is needed.
                                 @else   是否需要发送响应。 @endif */
    bool need_authorize;    /*!< @if Eng Whether authorization is needed.
                                 @else   是否需要授权。 @endif */
...
/**
 * @if Eng
 * @brief Callback invoked when receive read by type request.
 * @par Callback invoked when  receive read by type request.
 * @attention 1.This function is called in bts context,should not be blocked or do long time waiting.
 * @attention 2. The memories of <devices> are requested and freed by the bts automatically.
 * @param [in] server_id    server ID.
 * @param [in] conn_id      connection ID.
 * @param [in] read_cb_para read by type request parameter.
 * @param [in] status       error code.
 ...
 * @see gap_ble_callbacks_t
 * @else
 * @brief  收到基于类型读请求的回调函数。
 * @par    收到基于类型读请求的回调函数。
 * @attention  1. 该回调函数运行于bts线程，不能阻塞或长时间等待。
 * @attention  2. <devices>由bts申请内存，也由bts释放，回调中不应释放。
 * @param [in] server_id    服务端 ID。
 * @param [in] conn_id      连接 ID。
 * @param [in] read_cb_para 基于类型读请求参数。
 * @param [in] status       执行结果错误码。
 * @retval 无返回值。
 * @par 依赖:
 * @li  bts_def.h
 ...
 * @endif
 */
typedef void (*gatts_read_by_type_request_callback)(uint8_t server_id, uint16_t conn_id,
    gatts_req_read_by_type_cb_t *read_cb_para, errcode_t status);
...
typedef struct {
    ...
    gatts_read_by_type_request_callback read_by_type_request_cb; /*!< @if Eng Read by type request received callback.
                                                                      @else   收到远端基于类型读请求回调函数。 @endif */
} gatts_callbacks_t;
...
/**
 * @if Eng
 * @brief  Send indication or notification to remote device and update character value.
 * @par Description: Send indication or notification to remote device and update character value,
                     send status depend on character descriptor: client characteristic configuration.
                     Only update character value when conn_id is 0xFFFF(invalid conn id)
 * @param  [in] server_id server ID.
 * @param  [in] conn_id   connection ID.
 ...
 * @li bts_def.h
 * @else
 * @brief  向对端发送通知或指示,并更新Character值。
 * @par Description: 向对端发送通知或指示，具体发送状态取决于特征描述符：客户端特征配置;
                     如果conn_id为0xFFFF(无效值)时更新本端character值,并向所有连接设备发送数据。
 * @param  [in] server_id 服务端 ID。
 * @param  [in] conn_id   连接ID。
...
/**
 * @if Eng
 * @brief  Send indication or notification to remote device and not update character value.
 * @par Description: Send indication or notification to remote device and not update character value,
                     send status depend on character descriptor: client characteristic configuration.
                     Only update character value when conn_id is 0xFFFF(invalid conn id)
 * @param  [in] server_id server ID.
 * @param  [in] conn_id   connection ID.
 ...
 * @li bts_def.h
 * @else
 * @brief  向对端发送通知或指示,不更新Character值。
 * @par Description: 向对端发送通知或指示，具体发送状态取决于特征描述符：客户端特征配置。
 * @param  [in] server_id 服务端 ID。
 * @param  [in] conn_id   连接ID。
 * @param  [in] param     通知或指示参数。
 ...
 * @endif
 */
errcode_t gatts_notify_indicate_direct_send(uint8_t server_id, uint16_t conn_id, gatts_ntf_ind_t *param);
