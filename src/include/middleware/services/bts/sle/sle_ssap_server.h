...
/**
 * @if Eng
 * @brief  Register SSAP server multi-service callbacks.
 * @par Description: Register SSAP server multi-service callbacks.
 * @param  [in] server_id    server ID.
 * @param  [in] service_uuid service UUID.
 * @param  [in] func         callback function.
 * @retval error code.
 * @par Notes
 *     1. This interface is mutually exclusive with the ssaps_register_callbacks interface.
 *        Only one interface can be used.
 *     2. The current interface needs to be invoked before start service.
 * @par Depends:
 * @li sle_ssap_stru.h
 * @else
 * @brief  注册 SSAP server 多服务回调函数。
 * @par Description: 注册多服务回调函数。
 * @param  [in] server_id    服务端ID。
 * @param  [in] service_uuid 服务唯一标识。
 * @param  [in] func         回调函数。
 * @retval 执行结果错误码。
 * @par 注意事项
 *     1、与ssaps_register_callbacks接口互斥，只能使用一个；
 *     2、当前接口需要在启动服务之前调用；
 * @par 依赖：
 * @li sle_ssap_stru.h
 * @endif
 */
errcode_t ssaps_register_multi_callbacks(uint8_t server_id, sle_uuid_t *service_uuid, ssaps_callbacks_t *func);
