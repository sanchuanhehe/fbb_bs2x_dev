...
/**
 * @if Eng
 * @brief  extern announce parameter.
 * @else
 * @brief  拓展广播参数。
 * @endif
 */
typedef struct sle_adv_ext_param {
    uint8_t adv_filter_policy;                  /*!< @if Eng announce filter policy, { @ref sle_filter_policy_t }
                                                    @else 广播过滤参数, { @ref sle_filter_policy_t } @endif */
} sle_adv_ext_param_t;
...
typedef struct sle_announce_param {
    ...
    void *ext_param;                        /*!< @if Eng extend parameter, default value is NULL
                                                         { @ref sle_adv_ext_param_t }
                                                 @else   扩展设备公开参数, 缺省时置空
                                                         { @ref sle_adv_ext_param_t } @endif */
