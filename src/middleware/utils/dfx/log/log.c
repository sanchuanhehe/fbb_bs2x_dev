...
#if CHIP_SW39
    lb_header.time_us = (uint32_t)uapi_systick_get_us();
#else
...
static errcode_report_callback_t g_errcode_report_callback;
void uapi_errcode_register_report_callback(errcode_report_callback_t cb)
{
    if (cb == NULL) {
        return;
    }
    g_errcode_report_callback = cb;
}

void uapi_errcode_unregister_report_callback(void)
{
    g_errcode_report_callback = NULL;
}

void uapi_errcode_invoke_report(uint8_t module, errcode_t err)
{
    if (err != ERRCODE_SUCC && g_errcode_report_callback != NULL) {
        g_errcode_report_callback(module, err);
    }
}
#endif
