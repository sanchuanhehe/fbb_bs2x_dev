...
{
#ifdef LOSCFG_MUTEX_DEBUG
    osal_log("osal_mutex_init caller is:0x%x\r\n", (unsigned int)__builtin_return_address(0));
#endif
    if (mutex == NULL) {
...
{
#ifdef LOSCFG_MUTEX_DEBUG
    osal_log("osal_mutex_destroy caller is:0x%x\r\n", (unsigned int)__builtin_return_address(0));
#endif
    if (mutex == NULL) {
        osal_log("parameter invalid!\n");
        return;
...
int osal_mutex_trylock(osal_mutex *mutex)
{
    ...
    return (int)LOS_MuxPend((unsigned int)(UINTPTR)mutex->mutex, LOS_NO_WAIT);
}
