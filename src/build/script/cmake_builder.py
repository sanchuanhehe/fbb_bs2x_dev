...
class CMakeBuilder(BuildEnvironment):
    ...
    def build_target(self, target):
        ...
        if env.get('upg_pkg'):
            ...
            output_path = env.get_output_path()
            version_insert_script_path = os.path.join(root_path, 'tools', 'bin', 'sign_tool', 'cfbb_script', 'key_version.py')
            version_insert_function = 'insert_key_version_fota'
            exec_shell([self.python_path, version_insert_script_path, version_insert_function, target, output_path], None, True)
    ...
    def start(self, env, target_name, output_path, nhso=None, clean=None):
        ...
        # insert Version
        sign_path = os.path.join(root_path, 'build', 'config', 'target_config', chip, 'sign_config', f"{target_name}.cfg")
        version_insert_script_path = os.path.join(root_path, 'tools', 'bin', 'sign_tool', 'cfbb_script', 'key_version.py')
        version_insert_function = 'insert_key_version'
        exec_shell([self.python_path, version_insert_script_path, version_insert_function, target_name, output_path, sign_path], None, True)
