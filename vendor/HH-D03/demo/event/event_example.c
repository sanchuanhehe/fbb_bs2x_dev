...
void event_task_entry(void)
{
    ...
    // 创建任务1
    example_task1_info =
        osal_kthread_create((osal_kthread_handler)example_event_read, NULL, "example_task1", TASK_STACK_SIZE);
    ret = osal_kthread_set_priority(example_task1_info, READ_TASK_PRI_EVENT);
    if (ret != OSAL_SUCCESS) {
        printf("example_task1 creat failed\r\n");
    }
    example_task2_info =
        osal_kthread_create((osal_kthread_handler)example_event_write, NULL, "example_task2", TASK_STACK_SIZE);
